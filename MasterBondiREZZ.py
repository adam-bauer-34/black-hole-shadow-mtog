...
To switch between the radial infall or bondi model, see getGeodesicEvolutionPlus.py
...
zetas = [-0.20, -0.15, -0.10, -0.05, 0.05, 0.10, 0.15, 0.20] # choose range of values of zeta in modified gravity theory

gammas = [float(13 * 9.**(-1)), float(5 * 3.**(-1))]
rcrits = [10., 30.]

accword = "bondi" # chooses accretion model. 

N = 2000 # total number of geodesics to ray trace (YOU NEED AT LEAST 4)
...
for j in range(0, len(powers)):
    ...
    for k in range(0, len(zetas)):
        ...
        for h in range(0, len(rcrits)):
            rcrit = float(rcrits[h])

            for l in range(0, len(gammas)):
                gamma = float(gammas[l])

                print("we're on power " + str(power) + " and zeta equals " + str(zeta) + " and rcrit is " + str(rcrit) + " and gamma is " + str(gamma) ) # tells you where you're at in the simulation

                for i in range(0, N):
                    #print("We are on geodesic number " + str(i))
                    Namearray[i] = Geodesic(IVarray[i], equation_list, zeta, accword, power, rcrit, gamma) # make class instance for geodesic (i+1)
                ...
                if theory == "GR":
                    ...
                    filename = "iData_" + accword + "_" + theory + "_p" + str(power) + "_rcrit" + str(rcrit) + "_g" + str(g) + "_N" + str(N) + ".csv"
                
                elif theory == "REZZ" and zeta < 0:
                    filename = "iData_" + accword + "_" + theory + "_zneg" + str(abs(zeta)) + "_p" + str(power) + "_rcrit" + str(rcrit) + "_g" + str(g) + "_N" + str(N) + ".csv"
                
                else: 
                    filename = "iData_" + accword + "_" + theory + "_z" + str(abs(zeta)) + "_p" + str(power) + "_rcrit" + str(rcrit) + "_g" + str(g) + "_N" + str(N) + ".csv"
...
# START ENERGY/ANGULAR MOMENTUM PLOT
