...
from src.getGeodesicEvolution import getGeodesicEvolution
from src.getMiscFuncs import getAngularMomentum, getEnergy, getCheck1, getCheck2, getCheck3
from src.getGeodesicEvolutionPlus import getGeodesicEvolutionPlus
import random 
...
class Geodesic: 
    ...
    def __init__(self, given_vals, expressions, zeta, accword, power, rcrit, gamma):

        self.given_vals = given_vals # inital values 
        self.expressions = expressions # sympy functions 
        self.zeta = zeta # modified coupling parameter
        ...
        self.intensity = solve_ivp(fun=lambda t, y: getGeodesicEvolutionPlus(t, y, self.expressions, zeta, accword, power, rcrit, gamma), t_span =self.a_range, y0 =self.getIntensityICs(), method="RK45", events=getCheck3, dense_output = True, atol=self.atol, rtol=self.rtol)
    ...
    def getIC(self):
        ...
        return [kt0, kr0, 0, kp0, t0, r0, th0, p0, self.zeta]
